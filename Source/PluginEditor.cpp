...
T60SliderAttachment(audioProcessor.tree, "T60", T60Slider), sig0SliderAttachment(audioProcessor.tree, "Frequency Independent Damping", sig0Slider), sig1SliderAttachment(audioProcessor.tree, "Frequency Dependent Damping", sig1Slider),
...
{
    setSliderAndLabelRotary(T60Slider, T60Label, "T60");
    T60Slider.setTextValueSuffix(" s");
    addAndMakeVisible(T60Slider);
    addAndMakeVisible(T60Label);
...
void PlateMiniProjectAudioProcessorEditor::resized()
{
    ...
    //plateArea.removeFromLeft(plateArea.getWidth()*0.1);
    ...
    //excitationArea.removeFromTop(excitationArea.getHeight()*0.4);
