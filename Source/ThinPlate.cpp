...
ThinPlate::ThinPlate (double kIn) : k (kIn) // <- This is an initialiser list. It initialises the member variable 'k' (in the "private" section in OneDWave.h), using the argument of the constructor 'kIn'.
{
    ...
    //uStates = std::vector<std::vector<double>> (Nx, std::vector<double>(Ny, 0));
    
    // Initialise pointer vector
    //uNext.resize (3, nullptr);
    //u.resize (3, nullptr);
    //uPrev.resize(3, nullptr);
    //uStates = std::vector<std::vector<double>> (3, std::vector<double>(N, 0));
    ...
    //    for (int m = 0; m < Ny; ++i)
     //   {
     //       uNext[i][m] = uStates[i][m];
     //       u[i][m] = uStates[i][m];
     //       uPrev[i][m] = uStates[i][m];
      //  }
    //}
}
...
void ThinPlate::calculateScheme()
{
    ...
    for (int l = 2; l < Nx-2; ++l) // clamped boundaries
    {
        for (int m = 2; m < Ny-2; ++m) // clamped boundaries
        {
            ...
            //u[0][l+m*Nx] =
            //(2-20*muSq-4*S)*u[1][l + m*Nx]
            //+ (8*muSq+S) * (u[1][l+1 + m*Nx] + u[1][l-1 + m*Nx] + u[1][l + (m+1)*Nx] + u[1][l + (m-1)*Nx])
            //- 2*muSq * (u[1][l+1 + (m+1)*Nx] + u[1][l-1 + (m+1)*Nx] + u[1][l+1 + (m-1)*Nx] + u[1][l-1 + (m-1)*Nx])
            //- muSq * (u[1][l+2 + m*Nx] + u[1][l-2 + m*Nx] + u[1][l + (m+2)*Nx] + u[1][l + (m-2)*Nx])
            //+ (sigma0*k-1+4*S) * u[2][l + m*Nx]
            //- S * (u[2][l+1 + m*Nx] + u[2][l-1 + m*Nx] + u[2][l + (m+1)*Nx] + u[2][l + (m-1)*Nx])
            //+ J * malletForce;
        }
    }
    updateStates();
