...
{
    int  excF { 0 };
    float T60 { 0 }, sig0 { 0 }, sig1 { 0 }, lengthX { 0 }, lengthY { 0 }, excX { 0 }, excY { 0 }, lisX { 0 }, lisY { 0 }, thickness { 0 }, excT { 0 };
};
...
void PlateMiniProjectAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    //thinPlate -> updateParameters(chainSettings.T60, chainSettings.lengthX, chainSettings.lengthY, chainSettings.excX, chainSettings.excY, chainSettings.lisX, chainSettings.lisY, chainSettings.thickness, chainSettings.excF, chainSettings.excT);
...
ChainSettings getChainSettings(juce::AudioProcessorValueTreeState& tree)
{
    ...
    settings.T60 = tree.getRawParameterValue("T60")->load();
...
juce::AudioProcessorValueTreeState::ParameterLayout PlateMiniProjectAudioProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("T60", "T60", 0.1f, 20.f, 4.f));
    ...
    //layout.add(std::make_unique<juce::AudioParameterFloat>("Frequency Dependent Damping", "Frequency Dependent Damping", 0.0001f, 0.1f, 0.0005f));
    ...
    //layout.add(std::make_unique<juce::AudioParameterFloat>("Material density", "Material density", 0.1f, 4.f, 0.75f));
    //layout.add(std::make_unique<juce::AudioParameterFloat>("Poissons ratio", "Poissons ratio", 0.1f, 10.f, 0.3f));
    //layout.add(std::make_unique<juce::AudioParameterInt>("Youngs modulus", "Youngs modulus", 100000000000, 1000000000000, 200000000000));
